...
public class MasterServer : MonoBehaviour {
	...
	private void OnDisconnect (int connectionId) {
		...
		if (gameServers.Count > 0) {
			...
			if (gameServerDisconnected != null) {
				Debug.Log("GameServer Disconnected - Removing GameServer from gameServers list");
		...
		if (clients.Count > 0) {
			...
			if (clientDisconnected != null) {
				Debug.Log("Client Disconnected - Removing Client from clients list");
	...
	private void Receive_Data_GameServerConnected(int connectionId, string[] splitData) {
		if (VerifySplitData(connectionId, splitData, 1)) {
			...
			if (gameServers.Count == 0 || (gameServers.Exists(gs => gs.ipAddress == connectedIpAddress) == false && gameServers.Exists(gs => gs.connectionId == connectionId) == false)) {
				// Add a new GameServer to our list of GameServers
				Debug.Log("Added new GameServer to servers list [ipAddress: " + connectedIpAddress + "] [connectionId: " + connectionId + "]");
				gameServers.Add(new ConnectedGameServer(connectedIpAddress, connectionId, connectedPort));
			} else {
				Debug.LogError("Failed to add GameServer to servers list [ipAddress: " + connectedIpAddress + "] [connectionId: " + connectionId + "]");
			}
		}
	}
	private void Receive_Data_ClientConnected(int connectionId, string[] splitData) {
		if (VerifySplitData(connectionId, splitData, 1)) {
			...
			// TODO: potentially block players from connecting with the same ipAddress?

			if (clients.Count == 0 || clients.Exists(cl => cl.connectionId == connectionId) == false) {
				// Add a new Client to our list of Clients
				Debug.Log("Added new Client to clients list [connectionId: " + connectionId + "]");
	...
	#region Send Methods
	// Answers
	private void Send_Answer_GameServerDetails (int connectionId) {
